...
const TARGET = process.argv[2] || path.join(__dirname, 'operator-studio-12.html');
...
function splice(html, libSource, adapterSource, sourceName){
  ...
  // Use a replacer function so `$` sequences inside vendored libraries
  // (notably QR alphanumeric cases like `$'`) are inserted literally instead
  // of being treated as String.replace replacement tokens.
